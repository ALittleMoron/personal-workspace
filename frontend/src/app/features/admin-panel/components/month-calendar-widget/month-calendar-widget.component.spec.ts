...
describe('MonthCalendarWidgetComponent', () => {
  ...
  it('renders a semantic month grid with multiple linked entries and today highlighted', () => {
    ...
    expect(table.querySelector('a[href="/admin-panel/knowledge/dates/date-1"]')).not.toBeNull();
    expect(table.querySelector('a[href="/admin-panel/knowledge/people/person-1"]')).not.toBeNull();
  ...
  it('lists February 29 separately when the selected year is not leap', () => {
    ...
    expect(
      omitted.querySelector('a[href="/admin-panel/knowledge/people/leap-person"]'),
    ).not.toBeNull();
  });
