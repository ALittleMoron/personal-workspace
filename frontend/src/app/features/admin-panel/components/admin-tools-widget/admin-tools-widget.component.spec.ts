...
interface AdminToolsServiceMock {
...
describe('AdminToolsWidgetComponent', () => {
  ...
  let service: AdminToolsServiceMock;
  ...
  it('keeps the cache card stable while its initial request is pending', () => {
    ...
    expect(element('[data-testid="admin-tools-cache-clear"]')).toBeNull();
  });
  ...
  it('renders a failed initial request and replaces it with cache data after retry', () => {
    ...
    expect(element('[data-testid="admin-tools-cache-retry"]')).toBeNull();
  });
...
function cacheStatus(keyCount: number): AdminCacheStatus {
  return {
    enabled: true,
