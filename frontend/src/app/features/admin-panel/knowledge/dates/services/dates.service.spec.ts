...
describe('KnowledgeDatesService', () => {
  ...
  it('maps URL-backed list filters and clones nested response data', () => {
    ...
    const request = httpMock.expectOne((value) => value.url.endsWith('/api/admin/knowledge/dates'));
  ...
  it('uses the Dates CRUD contract and generic private attachment endpoints', () => {
    service
      ...
      .subscribe();
    const create = httpMock.expectOne((value) => value.url.endsWith('/api/admin/knowledge/dates'));
    ...
    const update = httpMock.expectOne((value) =>
      value.url.endsWith('/api/admin/knowledge/dates/date-1'),
    );
    ...
    const content = httpMock.expectOne((value) =>
      value.url.endsWith('/api/admin/knowledge/files/file-1/content'),
    );
