...
describe('ToolsService', () => {
  ...
  it('loads and maps the cache status returned by the protected endpoint', () => {
    ...
    const request = httpMock.expectOne((item) => item.url.endsWith('/api/tools/cache'));
  ...
  it('clears the cache through its explicit mutation endpoint', () => {
    ...
    const request = httpMock.expectOne((item) => item.url.endsWith('/api/tools/cache/clear'));
  ...
  it('starts and polls the same manual cache-warm operation', () => {
    ...
    const start = httpMock.expectOne((item) => item.url.endsWith('/api/tools/cache/warm'));
    ...
    const poll = httpMock.expectOne((item) =>
      item.url.endsWith('/api/tools/cache/warm/operation-1'),
    );
