...
import { UnsavedChangesService, UnsavedChangesSource } from './unsaved-changes.service';
...
describe('UnsavedChangesService', () => {
  ...
  beforeEach(() => {
    ...
    TestBed.configureTestingModule({
      providers: [UnsavedChangesService, { provide: I18nService, useValue: { translate } }],
    });
  ...
  it('fingerprints map entries independently of insertion order', () => {
    const current = signal<unknown>(
    ...
    current.set(new Map<string, unknown>([['first', 2]]));
    expect(scope.hasChanges()).toBe(true);
  });
  ...
  it('commits a source or the whole scope as the new baseline', () => {
    ...
    const scope = service.createScope(destroyRef);
    const firstRegistration: UnsavedChangesSource = scope.registerSource(first, signal(true));
  ...
  it('asks for localized confirmation only when changes exist', () => {
    ...
    expect(translate).toHaveBeenCalledWith('unsavedChanges.confirmDiscard');
