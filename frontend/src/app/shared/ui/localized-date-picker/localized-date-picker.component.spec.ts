...
describe('LocalizedDatePickerComponent', () => {
  ...
  it('rejects a nonempty malformed external ISO value', () => {
    expect(fixture.componentInstance.validate(new FormControl('2027-02-31'))).toEqual({
      dateInvalid: true,
    });
    expect(fixture.componentInstance.validate(new FormControl('not-a-date'))).toEqual({
      dateInvalid: true,
    });
  });
...
describe('LocalizedDatePickerComponent on the server', () => {
  it('does not call browser constraint-validation APIs without a browser window', async () => {
    await TestBed.configureTestingModule({
      imports: [LocalizedDatePickerComponent],
