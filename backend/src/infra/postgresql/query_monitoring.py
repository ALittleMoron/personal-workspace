...
from sqlalchemy.engine.interfaces import ExceptionContext, ExecutionContext
...
def install_query_monitoring(
    *,
    ...
    event.listen(engine.sync_engine, "handle_error", clear_failed_query_timer)


def clear_failed_query_timer(exception_context: ExceptionContext) -> None:
    connection = exception_context.connection
    if connection is not None:
        pop_query_start_time(conn=connection)
