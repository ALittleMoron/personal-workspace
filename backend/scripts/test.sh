...
ensure_backend_deps
load_backend_env_file "$TEST_ENV_FILE"
...
cleanup_pytest_template_database() {
    ...
    if ! run_with_test_env uv run python -m scripts.pytest_databases drop-template; then
        echo "Pytest template database cleanup failed for ${BACKEND_PYTEST_DB_TEMPLATE_ID}." >&2
        return 1
    fi
...
cleanup_test_resources() {
    ...
    cleanup_pytest_template_database || status=1
    cleanup_owned_test_db || status=1
    return "$status"
}
