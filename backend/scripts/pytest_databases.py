...
_TARGET_TEST_DATABASE_RE = re.compile(
    r"^personal_workspace_test(?:_gw[0-9]+|_template_[0-9a-f]{8})?$",
)
_TARGET_TEST_DATABASE_HOSTS = frozenset({"localhost", "127.0.0.1"})
_TARGET_TEST_DATABASE_USER = "personal_workspace_test"
...
def validate_test_database_name(database_name: str) -> None:
    if _TARGET_TEST_DATABASE_RE.fullmatch(database_name) is None:
        msg = f"Refusing to manage non-test database: {database_name}"
        raise ValueError(msg)


def validate_test_database_connection(*, database_name: str, host: str, user: str) -> None:
    validate_test_database_name(database_name)
    if host not in _TARGET_TEST_DATABASE_HOSTS:
        msg = f"Refusing to manage a test database on host: {host}"
        raise ValueError(msg)
    if user != _TARGET_TEST_DATABASE_USER:
        msg = f"Refusing to manage a test database as user: {user}"
        raise ValueError(msg)
