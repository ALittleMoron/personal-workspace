...
Use the GNU Affero General Public License version 3. The unchanged license text is preserved in [`../LICENSE.md`](../LICENSE.md) during the documentation-only stage. A canonical root `LICENSE` file with the same text must be added before the first code transfer.
...
- The temporary docs-only placement does not replace the conventional root license required before code publication.
- Dependency and asset license compatibility must be reviewed in the relevant JIT work package.
