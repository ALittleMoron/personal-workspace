...
The application is private but browser-accessible, so a static long-lived browser credential is not sufficient. The solution must support server-side revocation, bounded sessions, safe renewal, and protection for cookie-authenticated state changes without carrying over account management.
...
Argon2id parameters, session expiration, idle expiration, and rotation details must follow current OWASP guidance when the authentication work package is specified:
